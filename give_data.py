    ...
    try:
        ...
        def get_value(image_collection, timereducer,spacereducer, region, year):
            ...
            if isinstance(image_collection, ee.image.Image):
                    ...
                    print("The input is neither an Image nor an ImageCollection.")
        ...
        max_temp_data = get_value(max_temp_coll,ee.Reducer.max(),ee.Reducer.max(),region,year)['tmmx_max']
        min_temp_data = get_value(min_temp_coll,ee.Reducer.min(),ee.Reducer.min(),region,year)['tmmn_min']    
        ...
        for i in range(len(soilvalue_list)):
            if soilvalue_list[i] is not None:
                soilvalue_list[i] = str(round(soilvalue_list[i]/2,3)) + " %"
            else:
               soilvalue_list[i] = 'Data is not available'

        
        for i in range(len(climatevalue_list)):
            if climatevalue_list[i] is not None:
                climatevalue_list[i] = (str(round(climatevalue_list[i], 5))+" "+climatevalue_unit_list[i])
            else: 
                climatevalue_list[i] = 'Data is not available'
        

        modified_climate_key_data = dict(
            zip(climatekey_list, climatevalue_list))

        modified_soil_data = dict(
            zip(soilkey_list, soilvalue_list))
